...
from models.gine_model import GINEModel  # Ensure this file contains the GINE model definition
import utils
...
class GINETrainer:
    ...
    def train(self, train_loader, val_loader, optimizer, epochs=100, patience=15):
        best_val_pr = 0
        trigger = 0
        ...
        print("\n" + "=" * 60)
        print(f"🚀 Starting GINE Training - Epochs: {epochs}, Patience: {patience}")
        print("=" * 60 + "\n")
...
def run_gine_training(endpoint_dir, output_dir='saved_models/gine', epochs=100, patience=15):
    set_seed()
    ...
    trainer = GINETrainer(model, device)
    best_val_roc, history_df = trainer.train(train_loader, val_loader, optimizer, epochs=epochs, patience=patience)

    model_path = os.path.join(output_dir, f'models/{endpoint}.pt')
    torch.save(model.state_dict(), model_path)

    history_path = os.path.join(output_dir, 'history', f'{endpoint}_training_history.csv')
    ...
    for name, loader in [('train', train_loader), ('val', val_loader), ('test', test_loader)]:
        ...
        print(
            f"{name.upper()} - ROC-AUC: {metrics['roc_auc']:.4f}, PR-AUC: {metrics['pr_auc']:.4f}, F1: {metrics['f1']:.4f}")
...
if __name__ == '__main__':
    ...
    os.makedirs('saved_models/gine', exist_ok=True)
    os.makedirs('saved_models/gine/history', exist_ok=True)

    for endpoint_dir in os.listdir(base_data_dir):
        run_gine_training(os.path.join(base_data_dir, endpoint_dir), epochs=100, patience=15)